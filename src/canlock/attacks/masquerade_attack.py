...
class MasqueradeAttack(AttackBase):
    ...
    def __init__(
        self,
        attacker_source: Optional[int] = None,
        prob: float = 0.2,
        seed: Optional[int] = None,
    ) -> None:
        """Initialize the Masquerade attack module.
        
        Args:
            attacker_source: Source address to change.
            prob: Probability of replacing an address.
            seed: Optional seed to determine predictable randomness.
        """
    ...
    def _src(self, can_id: Optional[int]) -> Optional[int]:
        """Fetch the source address (LSB 8 bits) of a given CAN identifier.
        
        Args:
             can_id: Optional int that represents a CAN identifier.
             
        Returns:
            Extracted source ID logic or None if input was None.
        """
        if can_id is None:
            return None
    ...
    def apply(self, df: pd.DataFrame, target: Optional[int] = None) -> pd.DataFrame:
        """Apply masquerade logic through mutating target datasets.
        
        Args:
            df: The dataframe carrying CAN messages.
            target: The targeted SPN.
            
        Returns:
            The augmented dataframe holding mutated masquerading targets.
        """
        if self.prob <= 0:
            return df
        ...
        if target is None and target_pgn is None:
            ...
            pgns = [
                ...
                for x in df2["can_identifier"].tolist()
            ]
            df2["pgn"] = pgns
        ...
        for i in chosen:
            cid = (
                int(df2.loc[i, "can_identifier"])
                if df2.loc[i, "can_identifier"] is not None
                else 0
            )
            new_cid = (
                (cid & ~0xFF) | (self.attacker_source & 0xFF)
                if self.attacker_source is not None
                else cid
            )
            ...
            if target is not None:
                try:
                    with get_session() as s:
                        spn_def = s.exec(
                            select(SpnDefinition).where(
                                SpnDefinition.spn_identifier == target
                            )
                        ).first()
                    if spn_def:
                        try:
                            ...
                            orig_raw = None
                        if spn_def.is_analog and getattr(
                            spn_def, "analog_attributes", None
                        ):
                        ...
                        df2.loc[i, "payload"] = set_spn_bits(
                            df2.loc[i, "payload"], spn_def, new_raw
                        )
                except Exception:
                    pass
