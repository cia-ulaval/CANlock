...
class SuspensionAttack(AttackBase):
    ...
    def __init__(
        self,
        suspend_fraction: float = 0.1,
        tec_increment: int = 8,
        bus_off_threshold: int = 256,
        recovery_delay_s: float = 0.05,
        seed: Optional[int] = None,
    ) -> None:
        """Initialize the suspension attack configuration.
        
        Args:
            suspend_fraction: Overall scope size for the action.
            tec_increment: Value to increment the Transmit Error Collector per drop.
            bus_off_threshold: Treshold for triggering Bus-Off state.
            recovery_delay_s: Duration seconds to recover from off bus states.
            seed: Internal RNG generation logic.
        """
    ...
    def _get_pgn(self, can_id: Optional[int]) -> Optional[int]:
        """Fetch the PGN from the CAN ID formatted internally.
        
        Args:
            can_id: The ID numeric representation.
            
        Returns:
            The converted id map if able.
        """
        try:
            return (
    ...
    def apply(self, df: pd.DataFrame, target: Optional[int] = None) -> pd.DataFrame:
        """Strip away target rows from the trace dataset structure.
        
        Args:
            df: Target stream.
            target: Selected target component identifier to be pruned or dropped.
        ...
        if spn_def is not None:
            ...
            srcs = [
                df2.loc[i, "src"]
                for i in pgn_candidates
                if df2.loc[i, "src"] is not None
            ]
            if not srcs:
                return df2
            counts = Counter(srcs)
            ...
            victim_target_idxs = set(
                i for i in pgn_candidates if df2.loc[i, "src"] == victim_src
            )
            ...
            for idx in victim_all_idxs:
                ...
                if TEC >= self.bus_off_threshold and last_bus_off_time is not None:
                    ...
                    if (
                        current_time - last_bus_off_time
                    ).total_seconds() >= self.recovery_delay_s:
                        TEC = 0
                        last_bus_off_time = None
