...
class CnnLstmAutoencoder(pl.LightningModule):
    ...
    def __init__(self, n_features: int, seq_len: int, lr: float = 1e-3) -> None:
        super().__init__()
        if seq_len < 4:
        ...
        self.enc_lstm = nn.LSTM(
            64, 32, num_layers=1, batch_first=True, bidirectional=True
        )
        ...
        self.dec_lstm = nn.LSTM(
            32, 64, num_layers=1, batch_first=True, bidirectional=True
        )
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        z = nn.functional.interpolate(
            z, size=self.seq_len, mode="linear", align_corners=False
        )
